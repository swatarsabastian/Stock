...
create table if not exists public.sales (
  ...
  purchase_medium text not null default 'STORE',
...
alter table public.sales add column if not exists purchase_medium text not null default 'STORE';
...
create or replace function public.place_order(
  ...
  p_product text,
  p_quantity integer,
  p_purchase_medium text default 'STORE'
)
returns void
language plpgsql
as $$
...
begin
  ...
  insert into public.sales(sold_at, product, quantity, price_inr, cost_inr, status, purchase_medium)
  values (p_sold_at, p_product, p_quantity, v_price, v_cost, 'completed', upper(coalesce(p_purchase_medium, 'STORE')));

  perform public.log_operation(
    'place_order',
    p_product,
    jsonb_build_object('quantity', p_quantity, 'purchase_medium', upper(coalesce(p_purchase_medium, 'STORE')))
  );
end;
$$;
...
begin
  ...
  insert into public.sales(sold_at, product, size, quantity, price_inr, cost_inr, status, purchase_medium)
  values (
    p_sold_at,
    p_product,
    p_size,
    ...
    v_cost,
    'completed',
    upper(coalesce(p_purchase_medium, 'STORE'))
  );

  perform public.log_operation(
    'place_order_with_size',
    p_product,
    jsonb_build_object('size', p_size, 'quantity', p_quantity, 'purchase_medium', upper(coalesce(p_purchase_medium, 'STORE')))
  );
end;
$$;
